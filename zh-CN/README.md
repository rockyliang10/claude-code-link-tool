...
7. 如果需要复用 Skills，点击“选择模板”，选择一个已经配置好 Skills 的项目目录或 `.claude` 目录；选择后工具会自动勾选“启动前同步 Skills”。
...
工具可以在启动 Claude Code 前，把模板 `.claude` 目录里的可复用 Skills 同步到当前工作目录。首次使用需要手动选择模板目录，之后会保存这个选择。
